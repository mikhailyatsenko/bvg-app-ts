...
export const fetchArrivals = createAsyncThunk<ArivalsRawData, Stop, { rejectValue: string }>(
  "arrivals/fetchByStopId",
  async (selectedStop: Stop, thunkAPI) => {
    try {
      const response = await axios.get<ArivalsRawData>(
        `https://v6.bvg.transport.rest/stops/${selectedStop.id}/arrivals?duration=10`
      );
